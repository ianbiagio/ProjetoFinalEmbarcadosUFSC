...
db = new sqlite3.Database('./distancias.db', (err)=>{
    if (err){
...
db.run(`CREATE TABLE IF NOT EXISTS distancias
        (distancia INT)`, 
        [], (err) => {
           if (err) {
...
app.post('/Distancias', (req, res, next)=>{
    db.run(`INSERT INTO distancias(distancia) VALUES(?)`,
